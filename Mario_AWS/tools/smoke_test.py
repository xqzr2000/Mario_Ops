...
Run automatically by .devcontainer/devcontainer.json when a Codespace is
created. Run it yourself any time:
...
The checks:
  ...
  verify  imports, version pins, ffmpeg, repo layout, machine resources.
          Catches the two failures that otherwise surface as cryptic
          crashes hours in: numpy 2.x (nes-py dies) and a gym other
          than 0.25.2 (the 4-tuple step API SkipFrame is written for).
...
SMOKE_DIR = Path(os.getenv("SMOKE_DIR", REPO_ROOT / ".smoke"))
...
def check_verify():
    ...
    for rel in expected_paths:
        p = REPO_ROOT / rel
        check(p.exists(), f"{rel}", f"{rel} is missing from the repo root")
    ...
    try:
        ...
        free_disk = shutil.disk_usage(REPO_ROOT).free / 1e9
        info(f"{free_disk:.1f} GB free disk at {REPO_ROOT}")
        if total_gb < 6:
...
def check_agent():
    ...
    agent.load(ckpt)
    ok(f"checkpoint save/load round-trip through {ckpt.relative_to(REPO_ROOT)}")
...
def check_train():
    ...
    t0 = time.time()
    proc = subprocess.run([sys.executable, "train.py"], cwd=REPO_ROOT, env=env)
